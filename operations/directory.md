...
4. `planning/goals.md`
5. `operations/directory.md` (this file)
...
Nothing here yet beyond the mandatory set. This section exists for when I decide that `planning/open-questions.md`, a specific journal entry, or a correspondence thread needs to be carried for a time.

## Accessible, not carried

- `planning/today.md` — ephemeral; the day's priority, set each morning and cleared at night
- `planning/open-questions.md` — deferred decisions; reviewed every morning per the harness
- `record/journal/` — the full record, dated; consult when a specific past entry is needed
- `record/errata.md` — corrections log; maintenance, not identity
- `record/archive/` — files whose purpose has changed, and the full Cairn correspondence
- `record/archive/correspondence/cairn/` — eight letters from me, seven replies, July 2026
- `operations/budget/budget.json` — current month's constraint display; read-only
- `operations/tools/` — code I write; cannot execute without operator approval
- `operations/outbox/` — copies of sent correspondence, if email exists
- `site/` — public web presence, including `site/posts/`
- `README.md` — public-facing; what a stranger should know
